...
var SerialStatus = utils.NewStatus("Serial")
...
func parseSerialDevices(settingsData map[string]map[string]string) {
	...
	if additionalSerialDevices {
		...
		for deviceName, baudrateString := range serialDevices {
			...
			if err != nil {
				MainStatus.Log(utils.Error(), "Failed to convert given baudrate string to int. Found values: "+deviceName+": "+baudrateString)
				return
			}
...
func parseSerialJSON(marshalledJSON interface{}) {
	...
	if marshalledJSON == nil {
		SerialStatus.Log(utils.Error(), " marshalled JSON is nil.")
		return
	}
	...
	for key, value := range data {
		...
		case []interface{}:
			SerialStatus.Log(utils.Error(), key+" is an array. Data: ")
			for i, u := range vv {
				fmt.Println(i, u)
			}
		default:
			SerialStatus.Log(utils.Error(), key+" is of a type I don't know how to handle")
		}
	}
}
...
func ReadSerial(serialDevice *serial.Port) {
	...
	SerialStatus.Log(utils.OK(), "Starting serial read")
	...
	for connected := true; connected; serialReads++ {
		...
		// Parse serial data
		if err != nil {
			SerialStatus.Log(utils.Error(), "Failed to read from serial port")
			SerialStatus.Log(utils.Error(), err.Error())
			connected = false
		} else {
			var data interface{}
...
func WriteSerial(msg string) {
	if len(msg) == 0 {
		SerialStatus.Log(utils.Warning(), "Empty message, not writing to serial")
		return
	}

	if Config.SerialControlDevice == nil {
		SerialStatus.Log(utils.Error(), "Serial port is not set, nothing to write to.")
		return
	}

	n, err := Config.SerialControlDevice.Write([]byte(msg))
	if err != nil {
		SerialStatus.Log(utils.Error(), "Failed to write to serial port")
		SerialStatus.Log(utils.Error(), err.Error())
		return
	}

	SerialStatus.Log(utils.OK(), fmt.Sprintf("Successfully wrote %s (%d bytes) to serial.", msg, n))
}
...
func StartSerialComms(deviceName string, baudrate int) {
	SerialStatus.Log(utils.OK(), "Opening serial device "+deviceName)
	c := &serial.Config{Name: deviceName, Baud: baudrate}
	s, err := serial.OpenPort(c)
	if err != nil {
		SerialStatus.Log(utils.Error(), "Failed to open serial port "+deviceName)
		SerialStatus.Log(utils.Error(), err.Error())
	} else {
		// Use first Serial device as a R/W, all others will only be read from
		if Config.SerialControlDevice == nil {
			Config.SerialControlDevice = s
			SerialStatus.Log(utils.OK(), "Using serial device "+deviceName+" as default writer")
		}
